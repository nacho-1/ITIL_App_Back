...
pub struct RFCIncidentRelation {
    pub id: Uuid,
    pub rfc_id: Uuid,
...
pub struct RFCIncidentCreateset {
    pub incident_id: Uuid,
}
...
pub async fn load_all(
    rfc_id: Uuid,
    pool: &DbPool,
) -> Result<Vec<RFCIncidentRelation>, crate::Error> {
    let mut tx = pool.begin().await?;
    check_valid_rfc(rfc_id, &mut *tx).await?;
    let relations = sqlx::query_as!(
        RFCIncidentRelation,
        "
        SELECT id, rfc_id, incident_id
        FROM rfc_incident_relations
        WHERE rfc_id = $1",
        rfc_id
    )
    .fetch_all(&mut *tx)
...
pub async fn create(
    rfc_id: Uuid,
    createset: RFCIncidentCreateset,
    pool: &DbPool,
) -> Result<RFCIncidentRelation, crate::Error> {
    createset.validate()?;
    let mut tx = pool.begin().await?;
    check_valid_rfc(rfc_id, &mut *tx).await?;
    let created_relation = sqlx::query_as!(
        RFCIncidentRelation,
        "
        INSERT INTO rfc_incident_relations (rfc_id, incident_id)
        VALUES ($1, $2)
        RETURNING id, rfc_id, incident_id",
        rfc_id,
        createset.incident_id,
    )
    .fetch_one(&mut *tx)
    .await
    .map_err(|e| match e {
...
pub async fn delete(
    ...
    match sqlx::query!(
        "
        DELETE FROM rfc_incident_relations
        WHERE rfc_id = $1
        AND id = $2
        RETURNING id",
        rfc_id,
